...
function setup()
  local avg_speed = 40.22
  
  local max_speed = 53
  
  return {
    properties = {
      max_speed_for_map_matching      = max_speed, -- max moving speed, kph -> m/s
    ...
    vehicle_height = 1.5, -- in meters, 2.5m is the height of van
    vehicle_width = 1, -- in meters, ways with narrow tag are considered narrower than 2.2m

    -- Size of the vehicle, to be limited mostly by legal restriction of the way
    vehicle_length = 1, -- in meters, 4.8m is the length of large or familly car
    vehicle_weight = 500, -- in kilograms
    ...
    surface_speeds = {
      ...
      stone = 20,
      rocky = 20,
      sand = 0.01, -- could also simply avoid them
...
function handle_incline(profile, way, result, data)
  data.incline = way:get_value_by_key("incline")
  if (data.incline == "steep") then
    return false
  end
end
